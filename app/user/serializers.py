...
class UserSerializer(serializers.ModelSerializer):#modelserializer -> for models
    ...
    def update(self, instance, validated_data):
        ...
        password = validated_data.pop('password', None) #remove the password from dictionary
        user = super().update(instance, validated_data) #just call the base class super, laveraging its functionality
...
class AuthTokenSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        user = authenticate(
            request=self.context.get('request'), #this one here gives the input email and password sent from the form etc
            username=email,
            password=password,
        )
        if not user:
